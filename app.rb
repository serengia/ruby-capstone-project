...
class App
  ...
  def music_albums_list
    return puts "No music found" if @preserve.music_albums.empty?

    @preserve.music_albums.each_with_index do |album, index|
      puts "#{index + 1}) Genre: #{album.genre.name} On spotify: #{album.on_spotify} Publish date: #{album.publish_date}"
    end
  end
